...
kind-load:
	cd zarf/k8s/kind/users-pod; kustomize edit set image users-api-image=users-api-amd64:$(VERSION)
...
kind-apply:
	...
	kubectl wait --namespace=zipkin-system --timeout=240s --for=condition=Available deployment/zipkin-pod
	kustomize build zarf/k8s/kind/users-pod | kubectl apply -f -

kind-services-delete:
	kustomize build zarf/k8s/kind/users-pod | kubectl delete -f -
